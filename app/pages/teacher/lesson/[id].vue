...
const lessons: Record<number, Lesson> = {
  1: {
    ...
    notes: 'Группа сильная, можно углубиться в Promise.all и Promise.race. Петя Николаев отстаёт — уделить внимание. Показать реальный пример с параллельными запросами к API.'
  },
  2: {
    ...
    notes: 'Дарья Михайлова уже читала про ООП самостоятельно — можно дать ей дополнительную задачу с множественным наследованием. Темп группы средний.'
  },
  3: {
    ...
    notes: 'Первое занятие с группой. Темп вводный — не перегружать. Убедиться, что у всех настроен VS Code с Live Server.'
  },
  4: {
    ...
    notes: 'Группа новая, темп осторожный. Сделать акцент на *args/**kwargs — часто путают.'
  },
  5: {
    ...
    notes: 'Группа хорошо усвоила базовые промисы. Можно ускорить вводную часть и уделить больше времени практике с реальным API.'
  },
  6: {
    ...
    notes: 'Группа с опытом Vue — провести аналогию: компонент в React = компонент Vue, props = props. Акцент на отличия.'
  },
  7: {
    ...
    notes: 'Продвинутая группа. Если останется время — разобрать алгоритм Дейкстры. Акцент на сложность O(V+E).'
  },
  8: {
    ...
    notes: 'Финальное занятие. Атмосфера поддерживающая. Убедиться что у всех есть GitHub аккаунт. Несколько студентов могут не успеть — дать дополнительный день.'
  }
}
  ...
  <div class="lsn-page">
    ...
    <div class="lsn-cols">
      ...
      <div class="lsn-right">
        ...
        <div class="lsn-card lsn-notes-card" :style="{ borderLeftColor: lesson.color }">
          <div class="lsn-card__icon-wrap" :style="{ background: lesson.color + '20' }">
            <span class="lsn-card__icon">📝</span>
          </div>
          <p class="lsn-section-label">ЗАМЕТКИ ПРЕПОДАВАТЕЛЯ</p>
          <p class="lsn-notes">{{ lesson.notes }}</p>
        </div>
